 ...
 * @return array
 */
function wpd_block_social_share_socials() : array {
	return apply_filters( 'wpd-block-social-share-socials', [
		'facebook' => [
			'label' => esc_html__( 'Share on Facebook', 'wpd-blocks' ),
		...
		'twitter'  => [
			'label' => esc_html__( 'Share on Twitter', 'wpd-blocks' ),
		...
		'linkedin' => [
			'label' => esc_html__( 'Share on LinkedIn', 'wpd-blocks' ),
		...
		'email'    => [
			'label' => esc_html__( 'Share via email', 'wpd-blocks' ),
 ...
 * @return string
 */
function wpd_block_social_share_url_by_social( string $social, int $post_id = 0 ) : string {
	...
	switch ( $social ) {
		...
		default:
			return apply_filters( "wpd-block-social-share-$social-url", '', $post_id );
	}
}
 ...
 * @return string
 */
function wpd_block_social_share_block_render( array $block_attributes ) : string {
	$block_class_name = 'wpd-block-social-share';
...
function wpd_block_social_share_block_init() {
	register_block_type( __DIR__, [
		'render_callback' => 'wpd_block_social_share_block_render',
	] );
}

add_action( 'init', 'wpd_block_social_share_block_init' );

function wpd_block_social_share_enqueue_scripts() {
	wp_add_inline_script(
		'wpd-block-social-share-editor-script',
		'var wpdBlockSocialShareSocials = ' . wp_json_encode( wpd_block_social_share_socials() ),
		'before'
	);
}

add_action( 'admin_enqueue_scripts', 'wpd_block_social_share_enqueue_scripts' );
